...
- **GIVEN** que o motorista está com uma entrega em andamento
...
- **GIVEN** que o motorista está em trânsito com sua carga
...
- **GIVEN** que existe um ping "pendente" para o motorista com tempo de expiração no futuro
...
- **GIVEN** que existe um ping não respondido cujo `expires_at` é menor que o `now()` atual
...
- **GIVEN** que o motorista possui um ping não respondido cujo `expires_at` já passou
...
- **GIVEN** uma linha matemática predefinida entre a coordenada da Fábrica e da Loja
...
- **GIVEN** que a API possui múltiplos alertas de diferentes níveis e status
...
- **GIVEN** que o endpoint de alertas requer parâmetros de paginação
...
- **GIVEN** um alerta existente com o status "ativo"
